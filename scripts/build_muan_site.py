...
function show(i) {{
  ...
  if (p.installation_video) body += `<section class="section"><h3>安装视频</h3><video class="install-video" controls preload="metadata" playsinline src="${{esc(p.installation_video)}}"></video><div class="actions"><a class="action" href="${{esc(p.installation_video)}}" download="${{esc(p.code)}}-安装视频.mp4">下载安装视频</a></div></section>`;
