...
<body>
<header class="top"><div class="top-inner"><div class="eyebrow">MUAN PRODUCT LIBRARY</div><h1>{page_title}</h1><div class="search"><input id="query" placeholder="输入完整或部分货号，例如 6908" autocomplete="off"><button id="search">查询</button></div></div></header>
...
function show(i) {{
  ...
  if (p.installation_video) body += `<section class="section"><h3>安装视频</h3><video class="install-video" controls preload="metadata" playsinline src="${{esc(p.installation_video)}}"></video><div class="actions"><button class="action" id="copy-video">复制视频链接</button><a class="action" href="${{esc(p.installation_video)}}" download="${{esc(p.code)}}-安装视频.mp4">下载安装视频</a></div></section>`;
  body += `</div></div>`; $("#modal-body").innerHTML = body; $("#shade").classList.add("open");
  if (p.installation_video) $("#copy-video").addEventListener("click", async () => {{ const button = $("#copy-video"); try {{ await navigator.clipboard.writeText(new URL(p.installation_video, location.href).href); button.textContent = "已复制视频链接"; }} catch {{ button.textContent = "请手动复制地址"; }} }});
...
function filter(syncUrl = true) {{
  ...
  const compact = q.replace(/[^a-z0-9\u4e00-\u9fff]/g, "");
  const matches = products.map((p, index) => {{
    const code = String(p.code || "").trim().toLowerCase();
    const compactCode = code.replace(/[^a-z0-9\u4e00-\u9fff]/g, "");
    const other = [p.color,p.material,p.highlight].join(" ").toLowerCase();
    let rank = 99;
    if (code === q || compactCode === compact) rank = 0;
    else if (code.startsWith(q) || compactCode.startsWith(compact)) rank = 1;
    else if (code.includes(q) || (compact && compactCode.includes(compact))) rank = 2;
    else if (other.includes(q)) rank = 3;
    return {{ p, index, rank }};
  }}).filter(item => item.rank < 99).sort((a, b) => a.rank - b.rank || a.index - b.index).map(item => item.p);
  render(matches);
}}
